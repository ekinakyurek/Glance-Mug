var ads0x14 = require('node-ads1x15');
    ...
    if(!self.adc.busy){
		...
		self.adc.readADCSingleEnded(self.channels[self.current_channel_index], self.pga, self.sps, function(err, data) {
			...
			if (!self.waiting_for_click_multiple[current]){
				...
				if((current == 2 || current == 1) && result.equation[0] > 0.00049 && result.equation[2] > 0.97  && average > 0.1) {
					self.event.emit("swipe-up,"+ current.toString(), result.equation[0]);
					//console.log("swipe up")
					//console.log("slope: " + result.equation[0]);
					//console.log("correlation: " + result.equation[2]);
				}else if((current == 2 || current == 1) && result.equation[0] < -0.00049 && result.equation[2] < -0.97 && average > 0.1){
					self.event.emit("swipe-down,"+current.toString(), result.equation[0]);
					//console.log("swipe down")
					//console.log("slope: " + result.equation[0]);
					//console.log("correlation: " + result.equation[2]);
				}else if(current == 0 && result.equation[0] > 0.00045 && result.equation[2] > 0.91 && average < 1.0){
					self.event.emit("swipe-up,"+ current.toString(), result.equation[0]);

				}else if(current == 0 && result.equation[0] < -0.00055 && result.equation[2] < -0.95 && average < 1.0){
					self.event.emit("swipe-down,"+current.toString(), result.equation[0]);

				}else if((current == 2) && (result.equation[1] > 0.92|| result.equation[2]<-0.92) && (average > 0.99)){
